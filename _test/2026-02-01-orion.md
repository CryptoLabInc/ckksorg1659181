...
def construct_toeplitz_matrix(input_shape, output_shape, conv_layer):
    """
    Construct the Toeplitz representation of the convolutional layer.
    This matrix, when multiplied with the flattened input tensor,
    will produce the flattened output tensor.

    Args:
        input_shape (tuple): shape of the input tensor (N, Ci, Hi, Wi)
        output_shape (tuple): shape of the output tensor (N, Co, Ho, Wo)
        conv_layer (nn.Conv2d): convolutional layer

    Returns:
        K_Mat (torch.Tensor): matrix representation of the convolutional layer
    """
    ...
    H_padded_i = Hi + 2*P  # padded input image height
    W_padded_i = Wi + 2*P  # padded input image width

    # calculate the number of rows and columns in the kernel matrix
    n_rows = N * Co * Ho * Wo
    n_cols = N * Ci * H_padded_i * W_padded_i
    K_Mat = torch.zeros(n_rows, n_cols)

    # all indices of the padded input
    indices = torch.arange(Ci * H_padded_i * W_padded_i, dtype=torch.int32).reshape(Ci, H_padded_i, W_padded_i)
    starting_indices = indices[:Ci, :kH*dilation:dilation, :kW*dilation:dilation].flatten()
    starting_top_indices = indices[0, 0:Ho*S:S, 0:Wo*S:S].flatten()

    # fill kernel matrix with kernel values
    rows_to_fill = (torch.arange(Co) * Ho * Wo).reshape(Co, 1)
    for i, start_idx in enumerate(starting_top_indices):
        K_Mat[i + rows_to_fill, starting_indices + start_idx] = K.reshape(Co, -1)

    image_indices = indices[:, P:Hi+P, P:Wi+P].flatten()
    K_Mat = K_Mat[:, image_indices]
...
For the Toeplitz representation to be useful, we need better slot utilization for strided convolutions. Our key observation is that we can permute the matrix rows without changing the computation. All that changes is the order of the output feature map. Figure 5 shows this approach for a convolution with stride $s=2$.
