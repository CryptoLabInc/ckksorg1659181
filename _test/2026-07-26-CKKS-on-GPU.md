...
Conventional CPU-based HE libraries rely on ~60-bit RNS primes designed to match 64-bit CPU registers and SIMD vector extensions. On GPUs, however, the integer datapath and register file are optimized for 32-bit (INT32) execution. INT64 arithmetic is therefore implemented through software emulation, where multiple INT32 operations are combined to emulate 64-bit functionality. By instead using smaller RNS primes that fit within INT32, we can directly leverage the GPU's native integer execution units, achieving substantially higher throughput and efficiency than INT64 emulation.

A previous CKKS.org blog post already discussed a similar issue and introduced several approaches for constructing RNS using smaller word sizes such as INT32: [Modern Construction of Moduli Chain in HEaaN2](https://ckks.org/blog/2026/heaan2-moduli-chain/). Here, I focus only on Cheddar's **25-30 prime system**, which is specifically designed for GPU execution.
...
1. Add two new Pr~25 primes (inverse rescaling, which multiplies the polynomial by these two primes), then discard three Pr~30 primes (ordinary rescaling). The resulting scale adjustment is $25 + 25 - 30 - 30 - 30 = -40$ bits.
2. Add two new Pr~30 primes and discard four Pr~25 primes. The resulting scale adjustment is $30 + 30 - 25 - 25 - 25 - 25 = -40$ bits.

We then repeat

$$1 \rightarrow 1 \rightarrow 2 \rightarrow 1 \rightarrow 1 \rightarrow 2 \rightarrow \cdots$$

Throughout this cycle, the number of Pr~25 evolves as

$$0 \rightarrow 2 \rightarrow 4 \rightarrow 0 \rightarrow 2 \rightarrow 4 \rightarrow \cdots,$$
...
where $A$ is the number of Pr~25 ($\mathcal{T}_i$) and $B$ is the number of Pr~30 ($\mathcal{Q}_i$). A polynomial at this level therefore contains $A+B$ limbs, each consisting of $N$ INT32 coefficients corresponding to one prime.
...
context->MadUnsafe(tmp, ct2, constant_one);
...
Thanks to these optimizations, Theodosian achieves an additional 1.45–1.83$\times$ speedup over Cheddar on the RTX 5090.

<!--
| Implementation | Boot (ms) | HELR (ms/it) | ResNet-20 (ms) |
| :--- | :---: | :---: | :---: |
| FIDESlib | 147 | - | - |
| Cheddar | 22.1 | 25.9 | 720 |
| Theodosian | 15.2 | 14.1 | 467 |
| Theodosian (with CKKS bootstrapping algorithm enhancements) | 12.8 | - | - |
-->
...
<table style="border-collapse: collapse; border: 1px solid; min-width: 100%;">
  <thead>
    <tr style="border-bottom: 2px solid ">
      ...
      <th style="border: 1px solid; padding: 6px 12px;">ResNet-20 (ms)</th>
    </tr>
  </thead>
  <tbody>
    <tr>
      <td style="border: 1px solid; padding: 6px 12px; min-width: 80px;">FIDESlib</td>
    ...
    <tr>
      ...
      <td style="border: 1px solid; padding: 6px 12px; text-align: center;">720</td>
    </tr>
    <tr>
      ...
      <td style="border: 1px solid; padding: 6px 12px; text-align: center;"><strong>467</strong></td>
    </tr>
    <tr>
      ...
      <td style="border: 1px solid; padding: 6px 12px; text-align: center;"><strong>12.8</strong></td>
      <td style="border: 1px solid; padding: 6px 12px; text-align: center;">-</td>
      <td style="border: 1px solid; padding: 6px 12px; text-align: center;">-</td>
    </tr>
  </tbody>
