...
So, how can we efficiently compute polynomial evaluation? For polynomials with a sparse decomposition, such as $$f(X) = (x^3+1)^{16}\cdot(x^{32}+x-1)$$, we can compute the evaluation of each sparse-coefficient polynomial separately and then multiply them together. Thus, the computational cost is relatively low, although the multiplication depth may not be optimal. However, most polynomials are difficult to write in or do not have a sparse decomposition form. People can only use general polynomial evaluation algorithms to deal with them, among which the most well-known is the BSGS algorithm[^1].

Briefly speaking, given a polynomial $$f(X) = \sum_{i}f_{i}X^{i}$$ of degree $$d$$. The BSGS algorithm selects two integers, $$k\approx m\approx\sqrt{d}$$, such that $$km > d$$. Then the coefficients of $$f(X)$$ can be partitioned into $$m$$ intervals of length at most $$k$$, that is
...
First, we introduce "set multiplication", which is used to simplify the description of set decomposition in the algorithm. Given two sets $$S$$ and $$S'$$, which are subsets of a multiplicative group $$(G,\cdot)$$. Define $$S\times S':=\{s\cdot s'\mid s\in S, s'\in S'\} \subseteq G$$.

The BSGS algorithm can be rewritten as: Given $$S := \{1,x,\cdots,x^d\}$$, find two sets $$S_1$$ and $$S_2$$ of approximately the same size such that $$S\subseteq S_1\times S_2$$. We generalize it to:

$$
S\subseteq S_1^{(0)}\times\cdots\times S_1^{(t - 1)}\times S_2,
$$

such that 
$$|S_1^{(j)}| = O(d^{1/t})$$ and $$|S_2|\leq2^{t - 1}=O(1)$$, where $$t$$ is a small integer.

More importantly, we ensure that **the multiplicative depth of $$S_2$$ is $$t - 1$$ levels higher than that of $$S_1^{(j)}$$**. Then,
...
Then, it can be proven that $$\bar{S}_1\subseteq S_1^{(0)}\times\cdots\times S_1^{(t'-1)}$$, 
$$\hat{S}_1\subseteq S_1^{(t')}\times\cdots\times S_1^{(t - 1)}$$ and $$S_1\subseteq \bar{S}_1\times\hat{S}_1$$, where $$|\bar{S}_1| \approx |\hat{S}_1| = O(\sqrt{d})$$. Similar to the original BSGS algorithm, based on the pre-computed $$\bar{S}_1$$ and $$\hat{S}_1$$, the $$y_j$$ in step 2 can be calculated rapidly.
...
All homomorphic computing tasks that rely on high-degree dense polynomial evaluation will benefit from this, including leveled FHE bootstrapping and LWE amortized bootstrapping. And we achieve:

* $$2.9\times$$ improvement in throughput for the bootstrapping of BGV presented in paper[^4].
* $$3.5\times$$ increase in computational speed for the LWE amortized bootstrapping presented in paper[^5].
